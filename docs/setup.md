...
| `DROPBOX_CLIENT_ID`             | for Cloud Backup | Dropbox app key (App Console → App key). Backups write to `Sheltermark/Backups/` in the user's Dropbox.                                                                |
| `DROPBOX_CLIENT_SECRET`         | for Cloud Backup | Dropbox app secret.                                                                                                                                                    |
| `MS_CLIENT_ID`                  | for Cloud Backup | Microsoft (OneDrive) application (client) ID from Azure app registration, v2 endpoint, personal accounts.                                                              |
| `MS_CLIENT_SECRET`              | for Cloud Backup | Microsoft client secret. Redirect URI for all providers: `${NEXT_PUBLIC_SITE_URL}/api/backup/callback`.                                                                |
